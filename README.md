...
- Deployment-ready settings for SQLite in development and Neon PostgreSQL in production
...
For hosted deployments, point `DATABASE_URL` to the Neon connection string and keep SSL enabled with:
...
This repository supports Render hosting with a Neon PostgreSQL database in production. It also includes a `Dockerfile` for container-based deployment.

### Render

The included `render.yaml` configures a single web service with:
...
- `DEBUG`: `False`
- `ALLOWED_HOSTS`: your Render hostname, for example `jbac-llzt.onrender.com`
- `CSRF_TRUSTED_ORIGINS`: your HTTPS Render origin, for example `https://jbac-llzt.onrender.com`
- `DATABASE_URL`: Neon PostgreSQL connection string
- `DB_SSL_REQUIRE`: `True`
- `DB_SSLMODE`: `require`
- `DB_CONN_MAX_AGE`: `600`

On Render, make sure the Neon database is provisioned first, then copy its connection string into `DATABASE_URL`.
